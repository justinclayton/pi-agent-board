...
export class PtyAttachComponent implements Component {
	...
	// Lines scrolled per mouse-wheel event in the local attach viewport. Configurable via
	// AGENT_BOARD_WHEEL_LINES (legacy AGENT_VIEW_WHEEL_LINES); clamped to a sane range with a
	// low default so macOS momentum-scroll bursts don't rocket past the lines you wanted.
	private readonly wheelLines = resolveWheelLines(process.env.AGENT_BOARD_WHEEL_LINES ?? process.env.AGENT_VIEW_WHEEL_LINES);
	...
	private handleMouseInputChunk(events: Array<{ raw: string; mouse: { button: number; row: number; col: number; action: string } }>): boolean {
		...
		for (const entry of events) {
			...
			if ((mouse.button & 64) !== 0) {
				...
				this.clearPendingClick();
				this.clearSelection();
				if (!this.tryScrollBy(wheel * this.wheelLines)) this.send({ type: "input", data: raw });
				continue;
			}
			handled = true;
